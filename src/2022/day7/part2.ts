...
async function run() {
	...
	const sizeUnused = 7e7 - sizes[0];
	const sizeNeeded = 3e7 - sizeUnused;
	const valid = sizes.filter((size) => size >= sizeNeeded).sort((a, b) => a - b);

	console.log(`The directory that should be deleted has a size of ${bold(valid[0])}.`);
}
